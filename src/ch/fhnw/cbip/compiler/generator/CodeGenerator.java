...
public class CodeGenerator {
    ...
    public String generate() throws GenerationError {
        ...
        while (currentDecl != null) {
            if (declaration instanceof DeclFun)
                buildRoutine(declaration, true);
            currentDecl = currentDecl.getNextDecl();
        }
        ...
        currentDecl = declaration;
        while (currentDecl != null) {
            if (declaration instanceof DeclProc)
                buildRoutine(declaration, false);
            currentDecl = currentDecl.getNextDecl();
        }
    ...
    private void buildCmdAssi(CmdAssi cmd) throws GenerationError {
        if (cmd.getTargetExpr() instanceof ExprStore) {
            ...
            throw new GenerationError("wrong target expression for Cmd Assi");
    }
    ...
    /**
     * This builds a jump to the next command. Used for a Condition Command if the else command
     * isn't needed.
     * 
     * @param Cmd from the Abstract Tree
     */
    ...
    /**
     * This builds the code for a for loop with invariant.
     * 
     * @param Cmd from the Abstract Tree
     * @throws GenerationError
     */
    ...
    private void resolveExpression(Expr expr) throws GenerationError {
        ...
        if (expr instanceof ExprDyadic)
            resolveExprDyadic((ExprDyadic) expr);

        // calls a function
        else if (expr instanceof ExprFunCall)
            resolveExprFunCall((ExprFunCall) expr);

        // loads a literal
        else if (expr instanceof ExprLiteral)
            resolveExprLiteral((ExprLiteral) expr);

        // inverts the value of a expression
        else if (expr instanceof ExprMonadic)
            resolveExprMonadic((ExprMonadic) expr);

        // loads a variable from the store
        else if (expr instanceof ExprStore)
            resolveExprStore((ExprStore) expr);

        else
            throw new GenerationError("unknown expression");
    }
    ...
    private void resolveExprDyadic(ExprDyadic e) throws GenerationError {
        if (e.getOperator().getTerminal() == Terminal.BOOLOPR) {
            ...
            if (e.getOperator().getAttribute() == OperatorAttribute.COR)
                addLine("IntInv");
            ...
            case "MOD":
                ...
                break;
            default:
                addLine("Int" + e.getOperator().getAttribute());
                break;
            }
        } else
            throw new GenerationError("unknown terminal for a Dyadic Expression");
    }
    ...
    private void resolveExprMonadic(ExprMonadic e) throws GenerationError {
        if (e.getOperator().getTerminal() == Terminal.NOT
            ...
            throw new GenerationError("unknown prefix for a monadic expression");
    }
    ...
    private void addLine(String cmd, String params) {
        if (countingState)
            cmdCounter.set(cmdCounter.size() - 1, cmdCounter.get(cmdCounter.size() - 1) + 1);
        else {
            code.append("(");
     ...
     */
    private String getCallReplacement(String name) {
        return ">>" + name + "<<";
    }
}
