...
public class Parser {
    ...
    private ConcTree.Cmd cmd() throws GrammarError {
        ...
        case FOR:
            ...
            consume(Terminal.COLON);
            Type lcType = (Type) consume(Terminal.TYPE);
            if(lcType.getAttribute() != TypeAttribute.INT32)
                throw new GrammarError("Int32 excpected as for loop counter", lcType.getLine());
            consume(Terminal.SEMICOLON);
            Ident loopCounter2 = (Ident) consume(Terminal.IDENT);
            if(!loopCounter2.getName().equals(loopCounter.getName()))
                throw new GrammarError("Loop counter in init must be the same as declared before", 
            ...
            consume(Terminal.SEMICOLON);
            Ident loopCounter3 = (Ident) consume(Terminal.IDENT);
            if(!loopCounter3.getName().equals(loopCounter.getName()))
                throw new GrammarError("Loop counter in step must be the same as declared before", 
                        loopCounter2.getLine());
            consume(Terminal.BECOMES);
